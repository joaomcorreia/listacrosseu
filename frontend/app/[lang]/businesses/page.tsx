...
async function BusinessList({ searchParams, lang }: { searchParams: SearchParamsType, lang: Language }) {
    ...
    // Split businesses into featured (first 3) and free listings (remaining)
    const featuredBusinesses = businesses.slice(0, 3);
    const freeBusinesses = businesses.slice(3);
                ...
                <div className="mb-12">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-gray-900 flex items-center">
                            ...
                            {translate('businesses.featured_businesses', lang) || 'Featured Businesses'}
                        </h2>
                        <div className="w-16 h-0.5 bg-gradient-to-r from-yellow-400 to-orange-500"></div>
                    </div>
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-2xl font-bold text-gray-900">
                            {translate('businesses.free_listings', lang) || 'Free Listings'}
                        </h2>
                        <div className="text-sm text-gray-600">
                            {freeBusinesses.length} {translate('businesses.listings', lang) || 'listings'}
                        </div>
                    </div>
                    
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                        {freeBusinesses.map((business: Business) => (
                            <BusinessCard
                                key={business.slug}
                                business={business}
                                lang={lang}
                                variant="free-listing"
                            />
                        ))}
                    </div>
                </div>
