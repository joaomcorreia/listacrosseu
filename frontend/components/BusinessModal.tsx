...
export default function BusinessModal({ business, isOpen, onClose, lang }: BusinessModalProps) {
    ...
    // Generate consistent rating and reviews based on business data to avoid hydration issues
    const generateConsistentData = () => {
        const hash = business.name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
        const rating = (4.0 + (hash % 11) / 10).toFixed(1); // Results in ratings between 4.0 and 5.0
        const reviews = 25 + (hash % 50); // Results in reviews between 25 and 74
        return { rating, reviews };
    };

    const avatar = generateAvatar(business.name);
    const { rating, reviews } = generateConsistentData();
