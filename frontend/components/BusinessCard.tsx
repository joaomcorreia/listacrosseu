...
interface BusinessCardProps {
    business: Business;
    lang: Language;
    variant?: 'default' | 'featured' | 'compact' | 'free-listing';
}
...
export default function BusinessCard({ business, lang, variant = 'default' }: BusinessCardProps) {
    ...
    // Generate consistent rating based on business data to avoid hydration issues
    // In a real app, this would come from business.rating from the API
    const generateConsistentRating = () => {
        // Generate a deterministic "rating" based on business name to avoid hydration mismatch
        const hash = business.name.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
        const rating = 4.0 + (hash % 11) / 10; // Results in ratings between 4.0 and 5.0
        return rating.toFixed(1);
    };

    const rating = generateConsistentRating();
    ...
    if (variant === 'free-listing') {
        return (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 hover:shadow-md transition-all duration-200 hover:-translate-y-1">
                <div className="mb-3">
                    <h3 className="font-semibold text-gray-900 text-sm mb-2" style={{
                        display: '-webkit-box',
                        WebkitLineClamp: 2,
                ...
                >
                    {translate('business.claim_business', lang) || 'Claim this business'}
                </button>
    ...
    return (
        <div className={`bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-2 group ${variant === 'featured' ? 'ring-2 ring-blue-500/20' : ''
            }`}>
            {/* Header */}
                        ...
                        <div className="flex items-center space-x-2 mb-3">
                            <div className="flex items-center">
                                {[...Array(5)].map((_, i) => (
                                    <span
                                        key={i}
                                        className={`text-sm ${i < Math.floor(parseFloat(rating)) ? 'text-amber-400' : 'text-gray-300'}`}
                                    >
                                        ★
                                    </span>
                                ))}
